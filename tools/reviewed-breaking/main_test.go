...
func writeValidDeclaration(t *testing.T, path string, findings ...finding) {
	...
	value := declaration{
		...
		Against:         "v0.1.1",
		Review:          "TrueOpen/monorepo#74 (ADR-0013)",
...
func TestWriteAuthorsASortedDeclaration(t *testing.T) {
	...
	if err := run(findingsPath, declarationPath, declarationPath, "v0.2.0", "v0.1.1", "TrueOpen/monorepo#74", -1, "", false); err != nil {
...
func TestWrittenDeclarationMatchesThePublishedSchema(t *testing.T) {
	...
	if err := run(findingsPath, "", declarationPath, "v0.2.0", "v0.1.1", "TrueOpen/monorepo#74", -1, "", false); err != nil {
		t.Fatalf("write: %v", err)
	}
